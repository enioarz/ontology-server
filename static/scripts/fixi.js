(() => {
  ...
  document.addEventListener("fx:after", (evt) => {
    const headers = evt.detail.cfg.response.headers;
    const redirect = headers.get("fx-redirect");
    if (redirect) location.href = redirect;
    if (headers.get("fx-refresh")) location.reload();
    const retarget = headers.get("fx-retarget");
    if (retarget) evt.detail.cfg.target = document.querySelector(retarget);
    const reswap = headers.get("fx-reswap");
    if (reswap) evt.detail.cfg.swap = reswap;
    let select =
      headers.get("fx-reselect") || evt.target.getAttribute("fx-select");
    if (select) {
      const doc = parseHTML(evt.detail.cfg.text);
      const elt = doc.querySelect(select);
      if (elt) {
        evt.detail.cfg.text = elt[evt.detail.cfg.swap] || elt.outerHTML;
      } else {
        evt.preventDefault();
        return;
      }
    } else if (/<(html|body)(\s[^>]*)?>/i.test(evt.detail.cfg.text)) {
      // full page response instead of partial so remove head and replace all
      document.head.remove();
      evt.detail.cfg.target = docuement.body;
      evt.detail.cfg.swap = "outerHTML";

      // if you want to instead ignore full page responses uncomment:
      // evt.preventDefault();

      // if you want to instead just swap body contents to the target uncomment:
      // const doc = parseHTML(evt.detail.cfg.text);
      // const elt = doc.querySelect("body");
      // evt.detail.cfg.text = elt.innerHTML;
    }
  });
})();
